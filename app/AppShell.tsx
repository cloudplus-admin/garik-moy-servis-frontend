...
function DebtModal({company,role,close,done}:{company:Company;role:Role;close:()=>void;done:(text:string)=>void}){
 ...
 const [scope,setScope]=useState<"import"|"retail">(initialScope),[kind,setKind]=useState("receivable"),[customer,setCustomer]=useState(""),[store,setStore]=useState(""),[amount,setAmount]=useState(""),[dueDate,setDueDate]=useState("2026-08-20"),[manager,setManager]=useState(""),[comment,setComment]=useState(""),[saving,setSaving]=useState(false),[error,setError]=useState("");
 const details=useBusinessData(scope),managers=details.staff.filter(item=>!item.includes("Кладовщик")).map(item=>item.split(" · ")[0]);
 const selectedStore=store||details.warehouses[0]?.[0]||"",selectedManager=manager||managers[0]||"";
 function changeScope(value:string){setScope(value as "import"|"retail");setStore("");setManager("")}
 useEffect(()=>{const previous=document.body.style.overflow;document.body.style.overflow="hidden";const key=(event:KeyboardEvent)=>{if(event.key==="Escape")close()};window.addEventListener("keydown",key);return()=>{document.body.style.overflow=previous;window.removeEventListener("keydown",key)}},[close]);
 async function submit(){
  if(!customer.trim()||!amount.trim()||!dueDate||!selectedStore||!selectedManager){setError("Заполни контрагента, магазин, сумму, срок и ответственного");return}
  setSaving(true);setError("");
  const body={businessId:scope==="import"?1:2,kind,customer:customer.trim(),store:selectedStore,amount:amount.replace(/\D/g,""),dueDate:new Date(`${dueDate}T00:00:00`).toLocaleDateString("ru-RU"),manager:selectedManager,comment:comment.trim()};
