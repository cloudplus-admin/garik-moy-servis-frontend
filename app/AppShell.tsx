...
import {
  ...
  Eye, FileSpreadsheet, LayoutDashboard, Menu, Moon, Package, Plus, Sun,
  UserRound, Users, WalletCards, Warehouse, X
} from "lucide-react";
import { Select } from "./components/Select";
import { PageState } from "./components/PageState";
import { API_URL, invalidatePage, loadApplicationConfiguration, loadPage } from "./lib/api";
import {exportWordReport} from "./lib/export-word-report";
import type {ApplicationConfiguration,BusinessData,Company,ObserverProfile,Page,PageData,SalesTrendPoint} from "./lib/types";
...
const emptyBusinessData:BusinessData={hero:[],kpi:[],products:[],clients:[],movements:[],warehouses:[],sales:[],debts:[],cash:[],staff:[]};
...
export default function Home({section}:{section?:Page}){
  const pathname=usePathname(),router=useRouter();
  const routePage=(section??(pathname.split("/")[1]||"dashboard")) as Page;
  const page:Page=nav.some(item=>item.id===routePage)?routePage:"dashboard";
  ...
  const [observer,setObserver]=useState(""),[observerOpen,setObserverOpen]=useState(false),[dataExchangeOpen,setDataExchangeOpen]=useState(false),[debtOpen,setDebtOpen]=useState(false),[debtKind,setDebtKind]=useState<"receivable"|"payable">("receivable");
  const [toast,setToast]=useState(""),[menu,setMenu]=useState(false),[theme,setTheme]=useState<"light"|"dark">("light");
  const [salesFilter,setSalesFilter]=useState("");
  const [movementFilter,setMovementFilter]=useState("");
  const [selected,setSelected]=useState<BusinessData>(emptyBusinessData),[pageData,setPageData]=useState<PageData>([]),[pageDataKey,setPageDataKey]=useState(""),[loading,setLoading]=useState(true),[apiError,setApiError]=useState(""),[reloadNonce,setReloadNonce]=useState(0);
  const themeOwner=observer||"Алишер Юсупов";
  const observers=configuration?.observers??[];
  const currentUser=observer?observers.find(x=>x.name===observer):null;
  const pageConfiguration=configuration?.pages.find(item=>item.pageKey===page);
  const activeBusiness=company==="all"?null:configuration?.businesses.find(item=>item.code===company);
  const companyPresentation=company==="all"?{name:"Все компании",summary:"Группа компаний",code:"GROUP"}:{name:activeBusiness?.name??"Загрузка...",summary:activeBusiness?.summary??"Получаем данные компании",code:activeBusiness?.code.toUpperCase()??"..."};
  const businessOptions=[{value:"all",label:"Все компании"},...(configuration?.businesses.map(item=>({value:item.code,label:item.name}))??[])];
  useEffect(()=>{const controller=new AbortController();loadApplicationConfiguration(controller.signal).then(value=>{setConfiguration(value);setConfigurationError("")}).catch(error=>{if(error instanceof Error&&error.name!=="AbortError")setConfigurationError("Не удалось загрузить конфигурацию приложения")});return()=>controller.abort()},[]);
  ...
  return <div className="app">
    ...
    <main className="main">
      <div className="topbar">
        ...
        <Select ariaLabel="Выбрать бизнес" className={`company-switch company-${company}`} value={company} disabled={!!currentUser&&currentUser.company!=="all"} onChange={value=>{const next=value as Company;setCompany(next);window.localStorage.setItem(`garik-company:${themeOwner}`,next);navigate("dashboard")}} options={businessOptions}/>
      ...
      <div className={`contextbar company-${company}`} data-testid="business-context"><span className="context-code">{companyPresentation.code}</span><div><strong>{companyPresentation.name}</strong><small>{companyPresentation.summary}</small></div>{company==="all"&&<span className="consolidated">Консолидированные данные компаний из базы</span>}</div>
      <div className="pagehead"><div><h1 className="mid">{pageConfiguration?.title??"Загрузка..."}</h1><p>{pageConfiguration?.subtitle??"Получаем настройки из базы"} · {companyPresentation.name}</p></div><div className="page-actions"><button className="ghost data-exchange-trigger" onClick={()=>setDataExchangeOpen(true)}><FileSpreadsheet/>Импорт и экспорт</button><Action page={page} open={setModal} role={role} exportReport={()=>void exportWordReport(page,company,rows).then(()=>done("Отчет Word выгружен")).catch(()=>setToast("Не удалось выгрузить отчет"))}/></div></div>
      {configurationError?<PageState kind="error" message={configurationError} retry={()=>location.reload()}/>:apiError?<PageState kind="error" message={apiError} retry={()=>{invalidatePage(page,company);setReloadNonce(value=>value+1)}}/>:!configuration||loading||!pageDataReady?<PageState kind="loading"/>:page!=="dashboard"&&page!=="debts"&&rows.length===0?<PageState kind="empty"/>:<>
      ...
      {page==="clients"&&<ClientsPage
        rows={rows as string[][]} company={company} canCreate={role!=="Кладовщик"}
        onCreated={()=>done("Контрагент добавлен в базу")} onSales={openSales} onDebts={()=>navigate("debts")}
      />}
      {page==="debts"&&<DebtsPage
        canCreate={role!=="Кладовщик"} onCreate={kind=>{setDebtKind(kind);setDebtOpen(true)}} onPayment={()=>setModal("payment")}
        onMovements={openMovements} rows={rows as string[][]}
      />}
    ...
    {debtOpen&&<DebtModal company={company} role={role} initialKind={debtKind} close={()=>setDebtOpen(false)} done={text=>{setDebtOpen(false);done(text)}}/>}
    {observerOpen&&<ObserverModal observers={observers} current={observer} close={()=>setObserverOpen(false)} choose={impersonate}/>}
...
function DebtModal({company,role,initialKind,close,done}:{company:Company;role:Role;initialKind:"receivable"|"payable";close:()=>void;done:(text:string)=>void}){
 const initialScope: "import"|"retail"=company==="retail"?"retail":"import";
 const [scope,setScope]=useState<"import"|"retail">(initialScope),[kind,setKind]=useState(initialKind),[customer,setCustomer]=useState(""),[store,setStore]=useState(""),[amount,setAmount]=useState(""),[dueDate,setDueDate]=useState("2026-08-20"),[manager,setManager]=useState(""),[comment,setComment]=useState(""),[saving,setSaving]=useState(false),[error,setError]=useState("");
 const details=useBusinessData(scope),managers=details.staff.filter(item=>!item.includes("Кладовщик")).map(item=>item.split(" · ")[0]);
 const selectedStore=store||details.warehouses[0]?.[0]||"",selectedManager=manager||managers[0]||"",selectedCustomer=customer||details.clients[0]?.[0]||"";
 function changeScope(value:string){setScope(value as "import"|"retail");setStore("");setManager("")}
 useEffect(()=>{const previous=document.body.style.overflow;document.body.style.overflow="hidden";const key=(event:KeyboardEvent)=>{if(event.key==="Escape")close()};window.addEventListener("keydown",key);return()=>{document.body.style.overflow=previous;window.removeEventListener("keydown",key)}},[close]);
 async function submit(){
  if(!selectedCustomer||!amount.trim()||!dueDate||!selectedStore||!selectedManager){setError("Заполни контрагента, магазин, сумму, срок и ответственного");return}
  setSaving(true);setError("");
  const body={businessId:scope==="import"?1:2,kind,customer:selectedCustomer,store:selectedStore,amount:amount.replace(/\D/g,""),dueDate:new Date(`${dueDate}T00:00:00`).toLocaleDateString("ru-RU"),manager:selectedManager,comment:comment.trim()};
  try{const response=await fetch(`${API_URL}/api/debts`,{method:"POST",headers:{"Content-Type":"application/json","X-User-Role":encodeURIComponent(role)},body:JSON.stringify(body)});const result=await response.json().catch(()=>({}));if(!response.ok)throw new Error(result.message??"Не удалось добавить долг");done(kind==="payable"?"Кредит добавлен":"Дебит добавлен")}
  catch(reason){setError(reason instanceof Error?reason.message:"Не удалось добавить долг")}
  finally{setSaving(false)}
 }
 return <div className="modalback" onMouseDown={event=>{if(event.target===event.currentTarget)close()}}><section className="modal debt-modal" role="dialog" aria-modal="true" aria-labelledby="debt-modal-title"><div className="modalhead"><div><h2 id="debt-modal-title">{kind==="payable"?"Добавить кредит":"Добавить дебит"}</h2><small>{kind==="payable"?"Задолженность перед производителем":"Задолженность покупателя перед компанией"}</small></div><button className="close" aria-label="Закрыть окно" onClick={close}><X/></button></div><div className="form"><div className="formgrid">
  ...
  <div className="field"><label>Тип долга</label><Select ariaLabel="Тип долга" value={kind} onChange={value=>setKind(value as "receivable"|"payable")} options={[{value:"receivable",label:"Нам должен покупатель"},{value:"payable",label:"Мы должны производителю"}]}/></div>
  <div className="field"><label>{kind==="payable"?"Производитель или поставщик":"Покупатель"}</label><Select ariaLabel="Контрагент" value={selectedCustomer} onChange={setCustomer} options={details.clients.map(item=>({value:item[0],label:`${item[0]} · ${item[1]}`}))}/></div>
  <div className="field"><label>Магазин</label><Select ariaLabel="Магазин долга" value={selectedStore} onChange={setStore} options={details.warehouses.map(item=>({value:item[0],label:item[0]}))}/></div>
  ...
  <div className="field"><label>Ответственный</label><Select ariaLabel="Ответственный за долг" value={selectedManager} onChange={setManager} options={managers.map(item=>({value:item,label:item}))}/></div>
 ...
 </div>{error&&<p className="form-error" role="alert">{error}</p>}</div><div className="modalfoot"><button className="ghost" onClick={close}>Отмена</button><button className="primary" disabled={saving} onClick={()=>void submit()}><Plus/>{saving?"Сохраняем...":kind==="payable"?"Добавить кредит":"Добавить дебит"}</button></div></section></div>
}
function OperationModal({type,close,done,company}:{type:Exclude<Modal,null>;close:()=>void;done:(t:string)=>void;company:Company}){
 ...
 const [scope,setScope]=useState<"import"|"retail">(company==="retail"?"retail":"import"),d=useBusinessData(scope);
 const [saving,setSaving]=useState(false),[error,setError]=useState(""),[source,setSource]=useState(""),[destination,setDestination]=useState(""),[productId,setProductId]=useState(""),[quantity,setQuantity]=useState("24"),[transferDate,setTransferDate]=useState(new Date().toISOString().slice(0,10)),[responsible,setResponsible]=useState("");
 const sourceValue=source||d.warehouses[0]?.[0]||"",destinationValue=destination||d.warehouses[1]?.[0]||"",productValue=productId||d.products[0]?.[9]||"",responsibleValue=responsible||d.staff[0]?.split(" · ")[0]||"";
 async function submit(){const businessId=scope==="import"?1:2;if(type==="transfer"&&(sourceValue===destinationValue||!productValue||Number(quantity)<=0)){setError(sourceValue===destinationValue?"Выбери разные склады":"Выбери товар и укажи положительное количество");return}setSaving(true);setError("");const firstProductId=scope==="import"?1:5;const config=type==="sale"?{url:"/api/sales/new-sale",body:{businessId,customer:d.clients[0][0],warehouse:d.warehouses[0][0],store:d.warehouses[0][0],amount:"17 160 000",paymentStatus:"В долг",lines:[{productId:firstProductId,quantity:24},{productId:firstProductId+1,quantity:8}]}}:type==="payment"?{url:"/api/debts/payment",body:{debtId:scope==="import"?1:4,amount:"8 700 000"}}:{url:"/api/movements/transfer",body:{businessId,source:sourceValue,destination:destinationValue,productId:Number(productValue),quantity:Number(quantity),date:transferDate,responsible:responsibleValue}};try{const response=await fetch(`${API_URL}${config.url}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(config.body)});const result=await response.json().catch(()=>({}));if(!response.ok)throw new Error(result.message??"Сервер не сохранил операцию");done(type==="sale"?"Продажа проведена, партии списаны по FIFO":type==="payment"?"Оплата зарегистрирована, долг обновлен":"Перемещение создано, остатки обновлены")}catch(reason){setError(reason instanceof Error?reason.message:"Сервер не сохранил операцию. Проверь соединение и повтори.")}finally{setSaving(false)}}
 return <div className="modalback" onMouseDown={e=>{if(e.target===e.currentTarget)close()}}><div className="modal"><div className="modalhead"><div><h2>{cfg[0]}</h2><small>{company==="all"?"Выбери бизнес операции":company.toUpperCase()}</small></div><button className="close" aria-label="Закрыть окно" onClick={close}><X size={16}/></button></div><div className="form"><div className="formgrid">
 ...
 {type==="transfer"&&<><Field label="Склад-отправитель"><Select ariaLabel="Склад-отправитель" value={sourceValue} onChange={setSource} options={d.warehouses.map(x=>({value:x[0],label:x[0]}))}/></Field><Field label="Склад-получатель"><Select ariaLabel="Склад-получатель" value={destinationValue} onChange={setDestination} options={d.warehouses.map(x=>({value:x[0],label:x[0]}))}/></Field><Field label="Товар"><Select searchable ariaLabel="Товар" value={productValue} onChange={setProductId} options={d.products.map(x=>({value:x[9],label:x[1],secondary:`SKU: ${x[0]}${x[3]?` · ${x[3]}`:""}`,searchText:x[0]}))}/></Field><Field label="Количество"><input aria-label="Количество" type="number" min="1" value={quantity} onChange={e=>setQuantity(e.target.value)}/></Field><Field label="Дата отправки"><input aria-label="Дата отправки" type="date" value={transferDate} onChange={e=>setTransferDate(e.target.value)}/></Field><Field label="Ответственный"><Select ariaLabel="Ответственный" value={responsibleValue} onChange={setResponsible} options={d.staff.map(x=>x.split(" · ")[0]).map(x=>({value:x,label:x}))}/></Field></>}
...
function useBusinessData(scope:"import"|"retail"){
 const [value,setValue]=useState<BusinessData>(emptyBusinessData);
 useEffect(()=>{const controller=new AbortController();loadPage("dashboard",scope,controller.signal,true).then(result=>setValue(result as BusinessData)).catch(error=>{if(error instanceof Error&&error.name!=="AbortError")setValue(emptyBusinessData)});return()=>controller.abort()},[scope]);
 return value;
}

function ObserverModal({observers,current,close,choose}:{observers:ObserverProfile[];current:string;close:()=>void;choose:(name:string)=>void}){
 return <div className="modalback" onMouseDown={e=>{if(e.target===e.currentTarget)close()}}><div className="modal observer-modal"><div className="modalhead"><div><h2>Войти как пользователь</h2><small>Пароль сотрудника не требуется. Режим доступен директору и администратору.</small></div><button className="close" aria-label="Закрыть окно" onClick={close}><X size={16}/></button></div><div className="observer-intro"><Eye/><div><strong>Режим наблюдателя</strong><p>Меню, компания, права и действия изменятся точно так, как их видит выбранный сотрудник.</p></div></div><div className="observer-list">{observers.map(person=><button key={person.name} className={current===person.name?"selected":""} onClick={()=>choose(person.name)}><span>{person.name.split(" ").map(x=>x[0]).slice(0,2).join("")}</span><div><strong>{person.name}</strong><small>{person.title} · {person.role}</small></div><b className={`company-tag company-${person.company}`}>{person.company.toUpperCase()}</b></button>)}</div>{current&&<div className="modalfoot"><button className="ghost" onClick={()=>choose("")}>Вернуться в свой аккаунт</button></div>}</div></div>
}
