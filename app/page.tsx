...
import type {BusinessData,Company,Page,PageData} from "./lib/types";
...
export default function Home(){
  ...
  function openMovements(filter=""){setMovementFilter(filter);setSalesFilter("");navigate("products");setMenu(false)}
  function impersonate(name:string){
    const person=observers.find(x=>x.name===name);setObserver(name);
  ...
  return <div className="app">
    ...
    <main className="main">
      <div className="topbar">
        ...
        <Select ariaLabel="Выбрать бизнес" className={`company-switch company-${company}`} value={company} disabled={!!currentUser&&currentUser.company!=="all"} onChange={value=>{setCompany(value as Company);navigate("dashboard")}} options={[{value:"all",label:"Все компании"},{value:"import",label:"GARIK IMPORT"},{value:"retail",label:"GARIK RETAIL"}]}/>
        <button className="iconbtn" aria-label={theme==="light"?"Включить темную тему":"Включить светлую тему"} onClick={()=>setTheme(theme==="light"?"dark":"light")}>{theme==="light"?<Moon/>:<Sun/>}</button>
      ...
      {page==="warehouses"&&<WarehousesPage rows={rows as string[][]} company={company}/>}
...
function Dashboard({openPage,company,d}:{openPage:(p:Page)=>void;company:Company;d:BusinessData}){
  ...
  const periodCount=salesPeriod==="week"?7:12;
  const saleAmounts=d.sales.slice(0,periodCount).map(row=>Number((row[4]??"0").replace(/\D/g,"")));
  const maxSale=Math.max(...saleAmounts,1);
  const bars=Array.from({length:periodCount},(_,index)=>Math.max(8,Math.round((saleAmounts[index]??0)*100/maxSale)));
  const heroItems:[string,string,string,Page][]=[["Выручка",d.hero[1],"+12,6% за месяц","sales"],["Денежные средства",d.hero[2],"Кассы и счета","finance"],["Товаров на складах",d.hero[3],`${d.warehouses.length} точек хранения`,"products"],["Стоимость запасов",d.hero[4],"По текущей себестоимости","warehouses"]];
  ...
  <div className="grid2"><section className="card panel"><div className="panel-title"><div><h3>Динамика продаж</h3><span>Фактические продажи выбранного бизнеса</span></div><Select ariaLabel="Период динамики продаж" value={salesPeriod} onChange={setSalesPeriod} options={[{value:"week",label:"Неделя"},{value:"month",label:"Месяц"},{value:"year",label:"Год"}]}/></div><button className="dashboard-chart-button" onClick={()=>openPage("sales")} aria-label="Открыть продажи в кассе"><div className="chart">{bars.map((b,i)=><div className="barwrap" key={i}><div className="bar" style={{height:`${b}%`,animationDelay:`${i*35}ms`}}/><small>{i+1}</small></div>)}</div><span className="panel-link">Открыть продажи в кассе</span></button></section>
  <section className="card panel"><div className="panel-title"><div><h3>Требуют внимания</h3><span>{d.debts.filter((x:any)=>x[5]==="Просрочен").length+d.products.filter((x:any)=>x[7]==="Мало").length+1} события</span></div><button className="ghost" onClick={()=>openPage("debts")}>Открыть все</button></div><div className="alerts"><Alert kind="red" title={`${d.debts.filter((x:any)=>x[5]==="Просрочен").length} просроченных долга`} desc={`${d.debts.filter((x:any)=>x[5]==="Просрочен").reduce((s:number,x:any)=>s+Number(x[2].replaceAll(" ","")),0).toLocaleString("ru-RU")} сум в выбранном бизнесе`} value="Открыть" onClick={()=>openPage("debts")}/><Alert title={`Низкий остаток ${d.products.filter((x:any)=>x[7]==="Мало").length} товаров`} desc={d.products.find((x:any)=>x[7]==="Мало")?.[1]||"Критичных остатков нет"} value="Открыть" onClick={()=>openPage("products")}/><Alert title={d.movements[0]?.[0]||"Операций нет"} desc={`${d.movements[0]?.[1]||""} · ${companyInfo[company].name}`} value={d.movements[0]?.[5]||""} onClick={()=>openPage("movements")}/></div></section></div></>
}
