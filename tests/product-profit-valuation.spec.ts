...
test.beforeEach(async({page})=>{
 await page.route("**/api/**",async route=>{
  ...
  if(url.includes("/api/finance/overview")){const revenue=url.includes("period=week")?700000:url.includes("period=year")?12000000:300000;const product={sku:"IMP-LM-540",name:"Liqui Moly 5W-40",units:2,revenue,cost:200000,grossProfit:revenue-200000,margin:33.3,markup:50};return route.fulfill(json({available:1000000,payroll:0,supplierDebt:0,clientDebt:0,forecast:1000000,accounts:[],payrollItems:[],sales:[],products:[product],cashFlow:{income:revenue,expense:0,incomePercent:100,expensePercent:0,categories:[{type:"income",name:"Продажи",amount:revenue}]},profitAnalysis:{grossProfit:revenue-200000,totalSales:revenue,segments:[{name:"GARIK IMPORT",value:revenue,percent:100}],productBreakdown:[{business:"GARIK IMPORT",total:revenue,cost:200000,profit:revenue-200000,items:[{name:"Liqui Moly 5W-40",value:revenue,percent:100}],products:[product]}]},transactions:[]}));}
...
for(const width of [390,768,1280,1440])for(const theme of ["light","dark"] as const)test(`аналитика товара в кассе ${width}px ${theme}`,async({page})=>{
 ...
 if(theme==="dark")await page.getByRole("button",{name:"Включить темную тему"}).click();
 await expect(page.getByRole("heading",{name:"Анализ продаж"})).toBeVisible();await expect(page.getByRole("img",{name:/Доход от продаж по секциям.*GARIK IMPORT 100%/})).toBeVisible();await expect(page.getByRole("img",{name:/Продажи - GARIK IMPORT.*Liqui Moly 5W-40 100%/})).toBeVisible();
 await page.getByRole("button",{name:/Продажи - GARIK IMPORT/}).click();
...
test("период обновляет продажи, а диаграмма открывает панель секции",async({page})=>{
 await page.goto("/finance");
 const analysis=page.getByRole("heading",{name:"Анализ продаж"}).locator("..").locator("..");await expect(analysis).toContainText("300 000 сум");
 await page.getByRole("button",{name:"Период кассы"}).click();await page.getByRole("option",{name:"7 дней"}).click();
 await expect(analysis).toContainText("700 000 сум");
 await page.getByRole("button",{name:/Продажи - GARIK IMPORT/}).click();
 const dialog=page.getByRole("dialog",{name:"Продажи - GARIK IMPORT"});await expect(dialog).toContainText("700 000 сум");await expect(dialog).toContainText("Liqui Moly 5W-40");
});
